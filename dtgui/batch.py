...
class BatchProcessDialog(QtWidgets.QDialog):
    ...
    def __init__(self, baseline_params, *args, **kwargs):
        ...
        explanation = QtWidgets.QLabel(
            """Select files to be baseline-subtracted according to the parameters set in the previous screen. The files will be processed and stored in a directory of your choosing, along with a configuration file (.cfg) specifying the parameters used."""
        )
    ...
    def add_spectra(self):
        paths = QtWidgets.QFileDialog.getOpenFileNames(
            self, caption="Select one or more spectra", filter="*.csv"
        )[0]
    ...
    def accept(self):
        ...
        with open(
            os.path.join(directory, "baseline_parameters.txt"), mode="w"
        ) as configfile:
